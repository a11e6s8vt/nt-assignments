...
\begin{document}
	...
	\begin{enumerate}[1.]
        ...
        pub fn is_integer_of_form_pk_2pk(n: &BigInt) -> Vec<(BigInt, usize)> {
            ...
            \begin{flushleft}
                \textbf{\textit{Answer:}}
                From 2(c), it's evident that numbers of the form $p^k$ and $2p^k$ have primitive roots. A special case is when $p = 2$ and $p = 4$. When $p = 2$, $1$ is a primitive root. When $p = 4$, $\phi(4) = 2$ and $3^{\phi(4)} \equiv 1 \tpmod{4}$. Hence we can claim that if a number is of the form $2, 4, p^k,$ or $2p^k$ where $p > 2 and k \ge 1$, that prime has primitive roots.
            \end{flushleft}
		\end{enumerate}
