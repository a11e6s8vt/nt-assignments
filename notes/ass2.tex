...
\begin{document}
	...
	\begin{enumerate}[1.]
		...
		\begin{flushleft}
			...
			\bigbreak
			\begin{enumerate}
			\item \textbf{\textit{Answer:}} Given $a = 21, b = 47, n = 71$. We want to solve for $t$ in the congruence: $21^t \equiv 47\tpmod{71}$\\ \medbreak
			We have $\phi(71) = 70$. 
			\medbreak
			...
			\item We calculate $t = (mj + i)\tpmod{71} = (9 \times 5 + 2)\tpmod{71} = 47\tpmod{71}$\\
		...
		\item \textbf{\textit{Answer:}} Given $a = 26, b = 24, n = 53$. We want to solve for $t$ in the congruence: $26^t \equiv 24\tpmod{53}$\\ \medbreak
		 
		\medbreak
		\begin{enumerate}[Step 1.]
			\item Set m = $\lceil\sqrt{53}\rceil = 8$
			\item --
			\item Calculating $a^{mj}\tpmod{53}; 0 \le j < m$ \& Solve for $b.a^{-i}; 0 \le i < m$ (Step 2 and 3 tables below side-by-side)
